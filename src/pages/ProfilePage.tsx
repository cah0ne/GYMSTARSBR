...
export default function ProfilePage() {
  ...
  const fetchRefereeHistory = async () => {
    ...
    try {
      const q = query(
        ...
        where("refereeName", "==", userData.username),
      );
  ...
  const showEmail =
    userData?.email && !userData.email.includes("gymstars.internal");
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-[#0A1221] border border-slate-800 rounded-3xl p-8 flex flex-col items-center text-center">
            <div className="w-32 h-32 rounded-full flex items-center justify-center text-5xl overflow-hidden shadow-2xl border-4 border-white/5 mb-6 relative group bg-slate-900">
              {userData?.photoURL ? (
                <img
            ...
            {userData?.team && (
              <div className="mt-2 inline-flex items-center gap-1.5 px-3 py-1 bg-slate-800/50 border border-slate-700/50 rounded-full">
                ...
                <span className="text-xs font-bold text-slate-300">
                  EQUIPE {userData.team}
                </span>
              </div>
            )}
            {showEmail && (
              <p className="text-slate-500 text-sm mt-3">{userData?.email}</p>
            )}
          ...
          {isArbitro && (
            <div className="bg-[#0A1221] border border-slate-800 rounded-3xl p-6">
              <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                <Shield className="w-4 h-4 text-amber-500" /> Histórico de
                Arbitragem
              </h3>
              {loadingHistory ? (
                ...
                <div className="space-y-4 max-h-[300px] overflow-y-auto no-scrollbar">
                  {[...new Set(refereeHistory.map((h) => h.competitionId))].map(
                    (id) => {
                      const compId = id as string;
                      const compNotes = refereeHistory.filter(
                        (h) => h.competitionId === compId,
                      );
                      return (
                        <div
                          key={compId}
                          className="border-b border-slate-800/50 pb-3 last:border-0"
                        >
                          <div className="flex items-center justify-between mb-1 text-[10px] font-bold text-slate-300">
                            <span className="truncate">
                              Competição #{compId.substring(0, 5)}
                            </span>
                            <span className="text-amber-500">
                              {compNotes.length} notas
                            </span>
                          </div>
                          <div className="flex flex-wrap gap-1">
                            {[...new Set(compNotes.map((n) => n.category))].map(
                              (cat) => (
                                <span
                                  key={cat}
                                  className="px-1.5 py-0.5 bg-slate-900 border border-slate-800 rounded text-[9px] text-slate-400 font-mono"
                                >
                                  {cat}
                                </span>
                              ),
        ...
        <div className="lg:col-span-2">
          <div className="bg-[#0A1221] border border-slate-800 rounded-3xl p-8 h-full">
            ...
            {statusMessage && (
              <div
                className={`p-4 mb-6 rounded-2xl text-xs font-bold border flex items-center gap-3 animate-in fade-in slide-in-from-top-1 ${
            ...
            >
              ...
              {!isGinasta && !isVisitante && (
                <div
                  className={isVisitante ? "md:col-span-2" : "md:col-span-2"}
                >
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1">
