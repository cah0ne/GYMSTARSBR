...
import { db, collection, query, where, orderBy, onSnapshot, addDoc, updateDoc, deleteDoc, doc, setDoc } from "../lib/firebase";
...
export default function AboutPage() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, "appContent", "social_media_links"), (snap) => {
      if (snap.exists()) {
  ...
  const handleIconUpload = (e: React.ChangeEvent<HTMLInputElement>, isTiktok: boolean) => {
  ...
  return (
    <div 
      className="w-full min-h-screen px-4 py-8 flex flex-col items-center font-sans animate-fadeIn" 
    >
      ...
      {items.length === 0 ? (
        <div className="w-full max-w-3xl mx-auto px-4 mt-12">
          <div className="w-full bg-slate-900/40 border border-slate-800 rounded-3xl p-6 sm:p-12 text-center flex flex-col items-center justify-center gap-4">
            ...
            <h3 className="w-full text-xl font-bold text-slate-200">Em Construção</h3>
            <p className="w-full max-w-2xl mx-auto text-slate-400 text-sm leading-relaxed">
              As seções explicativas estão sendo escritas e editadas pelo administrador do site. Volte em breve para descobrir tudo sobre nós!
            </p>
            {isAdmin && (
              <button
                onClick={handleOpenAdd}
        ...
        {showEditorModal && createPortal(
          <div className="fixed inset-0 z-[9999] overflow-y-auto bg-black/85 flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleSave} className="w-full flex flex-col space-y-4 font-sans text-sm">
                  ...
                  {formPhotoBase64 ? (
                    <div className="w-full relative group border border-slate-800 rounded-2xl overflow-hidden bg-black h-36 flex items-center justify-center">
                      ...
                      >
                        ...
                        <span className="text-[10px] text-white font-bold uppercase tracking-widest text-center">Remover Imagem</span>
                      </button>
                    </div>
                  ) : (
                      ...
                      <label className="flex w-full flex-col items-center justify-center h-32 border border-slate-800 border-dashed rounded-2xl cursor-pointer bg-black/20 hover:bg-black/45 hover:border-slate-700 transition">
                        <div className="w-full flex flex-col items-center justify-center pt-5 pb-6">
                          ...
                          <p className="w-full text-center text-xs text-slate-400 font-bold px-2">
                            {uploadLoading ? "Redimensionando..." : "Selecionar foto"}
                          </p>
                        </div>
                        <input 
                          type="file" 
        ...
        {showSocialsModal && createPortal(
          <div className="fixed inset-0 z-[9999] overflow-y-auto bg-black/85 flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleSaveSocials} className="w-full flex flex-col space-y-5 text-sm mt-2">
                    ...
                    {formTiktokIcon ? (
                      <div className="flex items-center gap-4 bg-black/45 p-3 rounded-xl border border-slate-800">
                        <div className="relative w-12 h-12 bg-white/5 rounded-xl flex items-center justify-center border border-slate-800 p-1 shrink-0">
                          <img src={formTiktokIcon} alt="TikTok Icon Preview" className="w-full h-full object-contain" />
                          <button 
                            type="button" 
                        ...
                        <div className="text-[10px] text-slate-400 font-bold uppercase leading-relaxed shrink">
                          Ícone PNG Salvo. Toque no X para remover ou carregar outro.
                        </div>
                      </div>
                    ) : (
                      <div className="w-full">
                        <label className="flex items-center justify-center gap-2 p-3 border border-slate-800 border-dashed rounded-xl cursor-pointer bg-black/30 hover:bg-black/55 hover:border-slate-700 transition w-full text-center">
                          ...
                          <span className="text-xs text-slate-350 font-bold">
                            {tiktokUploadLoading ? "Compactando..." : "Upar Ícone em PNG"}
                          </span>
                          <input 
                            type="file" 
                            accept="image/png" 
                ...
                <div className="w-full space-y-2.5 p-4 rounded-2xl bg-black/20 border border-slate-800">
                  ...
                  <div className="w-full flex flex-col space-y-1">
                    ...
                    {formYoutubeIcon ? (
                      <div className="flex items-center gap-4 bg-black/45 p-3 rounded-xl border border-slate-800">
                        <div className="relative w-12 h-12 bg-white/5 rounded-xl flex items-center justify-center border border-slate-800 p-1 shrink-0">
                          <img src={formYoutubeIcon} alt="YouTube Icon Preview" className="w-full h-full object-contain" />
                          <button 
                            type="button" 
                      ...
                      <div className="w-full">
                        <label className="flex items-center justify-center gap-2 p-3 border border-slate-800 border-dashed rounded-xl cursor-pointer bg-black/30 hover:bg-black/55 hover:border-slate-700 transition w-full text-center">
                          <Upload className="w-4 h-4 text-slate-500" />
                          <span className="text-xs text-slate-350 font-bold">
                            {youtubeUploadLoading ? "Compactando..." : "Upar Ícone em PNG"}
                          </span>
                          <input 
                            type="file" 
                            accept="image/png" 
                ...
                <div className="w-full flex flex-col gap-2 pt-4 border-t border-slate-800 shrink-0">
                  <button
                    type="submit"
                    disabled={savingSocials || tiktokUploadLoading || youtubeUploadLoading}
