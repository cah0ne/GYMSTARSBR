...
const CATEGORIES = [
  ...
  "Avisos oficiais",
  "Área dos árbitros",
  "Dúvidas dos ginastas",
  "Treinos e rotinas",
  "Conversa geral",
];
const PRESET_COLORS = [
  { name: "Verde Esmeralda", class: "#009c3b" },
  { name: "Azul Radiante", class: "#3b82f6" },
  { name: "Ouro Real", class: "#f59e0b" },
  { name: "Rosa Vibrante", class: "#ec4899" },
  { name: "Roxo Místico", class: "#a855f7" },
  { class: "#06b6d4" },
  { name: "Vermelho Fogo", class: "#ef4444" },
];
...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    if (
      !userData ||
      !userData.tag ||
      !["Ginasta", "Árbitro", "Admin"].includes(userData.tag || "")
    ) {
      navigate("/");
    }
  }, [userData, navigate]);
  ...
  const [userBalloonColor, setUserBalloonColor] = useState(
    () => localStorage.getItem("chat_balloon_color") || "#009c3b",
  );
  ...
  const [roomForm, setRoomForm] = useState({
    ...
    allowedTags: ["Ginasta", "Árbitro", "Admin"] as any[],
    isTemporary: false,
  });
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        const item = doc.data();
        if (
          (item.allowedTags?.includes?.(userData.tag) ||
            userData.tag === "Admin") &&
          !item.kickedUsers?.includes?.(userData.uid)
        ) {
  ...
  useEffect(() => {
    ...
    const unsubPres = onSnapshot(qPres, (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        if (doc.data().lastActive > threshold)
          list.push(doc.data() as ChatPresence);
      });
    ...
    return () => {
      ...
      unsubPres();
      deleteDoc(doc(db, "chat_presence", presenceId)).catch(() => {});
    };
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snapshot) => {
      ...
      if (
        newLen > prevMessagesLength.current ||
        lastId !== prevLastMsgId.current
      ) {
        setTimeout(
  ...
  useEffect(() => {
    if (messages.length > 0 && userData && activeRoomId) {
      ...
      if (lastMsg.senderId !== userData.uid && lastMsg.senderId !== "system") {
        const hasRead = Array.isArray(lastMsg.readBy)
          ? lastMsg.readBy.some((r) => r.uid === userData.uid)
          : false;
        if (!hasRead) {
          const newReadEntry = {
            uid: userData.uid,
          ...
          const updatedReadBy = Array.isArray(lastMsg.readBy)
            ? [...lastMsg.readBy, newReadEntry]
            : [newReadEntry];
          updateDoc(doc(db, "chat_messages", lastMsg.id), {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        const d = doc.data();
        if (d.userId !== userData.uid && now - d.updatedAt < 5000)
          activeTyping.push(d as ChatTyping);
      });
  ...
  const renderMessages = useMemo(() => {
    ...
    return filteredMessages.map((msg, idx) => {
      ...
      const showHeader =
        ...
        filteredMessages[idx - 1]?.createdAt < msg.createdAt - 300000;

      if (isSystem) {
        const isScore =
          msg.text &&
          typeof msg.text === "string" &&
          (msg.text.includes("RESULTADO") || msg.text.includes("NOTA"));
        ...
        if (isScore) {
          ...
          if (isRealTimeScore) {
            ...
            lines.forEach((l) => {
              if (l.includes("Ginasta:"))
                gymnastName = l.replace(/.*Ginasta:\*\*/, "").trim();
              if (l.includes("Categoria:"))
                category = l.replace(/.*Categoria:\*\*/, "").trim();
              if (l.includes("Nota:"))
                score = l.replace(/.*Nota:\*\*/, "").trim();
            });
          }
          ...
          return (
            <div
              key={msg.id}
              className="flex justify-center my-6 w-full relative z-10"
            >
                ...
                <div className="p-4 flex flex-col gap-3">
                  <div className="flex items-center gap-3 pb-3 border-b border-white/10">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-black text-emerald-400 uppercase tracking-[0.15em] text-[10px] sm:text-xs mb-0.5">
                        Sistema de Notas
                      </h4>
                      <p className="text-white font-medium text-xs sm:text-sm bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent truncate">
                  ...
                  {isRealTimeScore && gymnastName ? (
                    <div className="grid grid-cols-2 gap-2 sm:gap-3 mt-1">
                      <div className="bg-black/40 rounded-xl p-3 border border-white/5 flex flex-col justify-center">
                        <span className="text-[9px] sm:text-[10px] uppercase tracking-wider text-emerald-500/80 font-bold mb-1">
                          Ginasta
                        </span>
                      ...
                      <div className="bg-black/40 rounded-xl p-3 border border-white/5 flex flex-col justify-center">
                        <span className="text-[9px] sm:text-[10px] uppercase tracking-wider text-cyan-500/80 font-bold mb-1">
                          Categoria
                        </span>
                        <span className="text-xs sm:text-sm font-semibold text-white truncate">
                          {category}
                      ...
                      <div className="col-span-2 bg-gradient-to-br from-emerald-500/20 to-[#009c3b]/10 rounded-xl p-4 sm:p-5 border border-emerald-500/20 flex flex-col items-center justify-center shadow-inner mt-1 relative overflow-hidden">
                        ...
                        <span className="text-[10px] sm:text-xs uppercase tracking-[0.2em] text-emerald-400/90 font-black mb-1 relative z-10">
                          Nota Final
                        </span>
                        <span className="text-3xl sm:text-4xl font-mono font-black text-white drop-shadow-[0_0_15px_rgba(16,185,129,0.5)] relative z-10">
                          {score}
                        </span>
                      </div>
                    </div>
      ...
      return (
        ...
        >
          {showHeader && (
            ...
            >
              {msg.senderPhotoURL ? (
                ...
                <div
                  className={`w-5 h-5 sm:w-6 sm:h-6 rounded-full flex items-center justify-center text-[10px] font-bold text-white shrink-0 ${isMe ? "bg-[#009c3b]" : "bg-slate-700"}`}
                >
                  {msg.senderName?.[0]?.toUpperCase()}
                </div>
              )}
              <div
                className={`flex items-center gap-1.5 sm:gap-2 ${isMe ? "flex-row-reverse" : ""}`}
              >
                <span
                  className={`text-[11px] sm:text-xs font-bold ${isMe ? "text-[#009c3b]" : "text-emerald-400"}`}
                >
                  {msg.senderName}
                </span>
                {msg.senderTag && (
                  <span className="text-[9px] sm:text-[10px] bg-white/10 px-1.5 rounded text-slate-300">
                    {msg.senderTag}
                  </span>
                )}
          ...
          <div
            className={`relative group flex ${isMe ? "flex-row-reverse" : "flex-row"} items-start gap-2 max-w-full`}
          >
            <div
              style={{
                backgroundColor:
                  msg.senderBalloonColor || (isMe ? "#009c3b" : "#1E293B"),
                boxShadow: msg.senderBalloonColor
                  ? `0 4px 14px ${msg.senderBalloonColor}22`
                  : undefined,
              }}
              className={`
            ...
            >
              {msg.replyToId && (
                <div
                  className={`mb-1.5 p-1.5 sm:p-2 rounded-lg text-[10px] sm:text-xs border-l-2 ${isMe ? "bg-black/20 border-white/50 text-emerald-50" : "bg-black/30 border-[#009c3b] text-slate-300"} flex flex-col gap-0.5`}
                >
              ...
              {msg.reactions && Object.keys(msg.reactions).length > 0 && (
                <div
                  className={`flex flex-wrap gap-1 mt-1 ${isMe ? "justify-end" : "justify-start"} -mb-2 z-10`}
                >
                  {Object.entries(msg.reactions)
                    .filter(([_, uids]) => (uids as string[]).length > 0)
                    .map(([emoji, uids]) => (
                      <button
                        key={emoji}
                        ...
                        className={`flex items-center gap-1 text-[10px] sm:text-xs font-medium px-2 py-0.5 rounded-full border transition-colors ${
                          Array.isArray(uids) &&
                          (uids as string[]).includes(userData!.uid)
              ...
              {idx === messages.length - 1 &&
                ...
                msg.readBy.filter((r) => r.uid !== msg.senderId).length > 0 && (
                  <div
                    className={`flex items-center -space-x-1 mt-1.5 ${isMe ? "justify-end pr-2" : "justify-start pl-2"}`}
                  >
                    <Check className="w-3 h-3 text-[#009c3b] mr-1" />
                    {msg.readBy
                      .filter((r) => r.uid !== msg.senderId)
                      .map((reader, i) => (
                        <div
                          key={reader.uid}
                          className="w-3.5 h-3.5 rounded-full border border-[#050B14] bg-[#1E293B] shadow-sm flex items-center justify-center overflow-hidden shrink-0 relative"
                          style={{ zIndex: 10 - i }}
                          title={`Visto por ${reader.username}`}
                        >
                          {reader.photoURL ? (
                            <img
                              src={reader.photoURL || undefined}
                              alt={reader.username}
  ...
  const handleSendMessage = async (
    ...
    if (
      ...
      return;
    if (activeRoom?.mutedUsers?.includes?.(userData.uid)) {
      alert("Você foi silenciado.");
      return;
    }
    ...
    try {
      ...
      await updateDoc(doc(db, "chat_rooms", activeRoomId), {
        lastMessageText: isAnnouncement
          ? `📣 [Aviso] ${textToSend}`
          : textToSend,
        lastMessageAt: Date.now(),
        lastMessageAuthorName: userData.username,
      });
      setReplyingTo(null);
      deleteDoc(
        doc(db, "chat_typing", `${activeRoomId}_${userData.uid}`),
      ).catch(() => {});
    } catch (err: any) {
  ...
  const handleToggleReaction = async (message: ChatMessage, emoji: string) => {
    ...
    try {
      const currentReactions = message.reactions
        ? { ...message.reactions }
        : {};
      const emojiUids = currentReactions[emoji]
  ...
  const handleCreateRoomSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!userData || (userData.tag !== "Admin" && userData.tag !== "Árbitro"))
      return;
    try {
      ...
      setRoomForm({
        ...
        color: "#009c3b",
        category: "Conversa geral",
        allowedTags: ["Ginasta", "Árbitro", "Admin"],
        isTemporary: false,
      });
    } catch (err: any) {}
  };
              ...
              <div key={category} className="space-y-2">
                ...
                <div className="space-y-1">
                  {catRooms.map((room) => (
                    ...
                    >
                      <div
                        className="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center shrink-0"
                        style={{
                          color: room.color,
                          border: `1px solid ${room.color}40`,
                        }}
                      >
                        {room.category === "Área dos árbitros" ? (
                      ...
                      <div className="flex-1 min-w-0 flex flex-col">
                        <div className="flex justify-between items-baseline mb-0.5">
                          ...
                          {room.isTemporary && (
                            <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse ml-2 shrink-0"></span>
                          )}
                        </div>
                        <p className="text-xs text-slate-400 truncate mb-2">
                          {room.lastMessageText
                            ? `${room.lastMessageAuthorName}: ${room.lastMessageText}`
                            : room.description || "Nenhuma mensagem ainda"}
                        </p>
                        <button
              ...
              >
                <header className="pt-[max(0.75rem,env(safe-area-inset-top))] pb-3 sm:py-4 border-b border-white/5 flex items-center justify-between px-3 sm:px-6 bg-[#070F1C] shrink-0 select-none relative z-10 shadow-md">
                  <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                    ...
                    <div
                      className="hidden xs:flex items-center justify-center w-8 h-8 sm:w-9 sm:h-9 rounded-full shrink-0"
                      style={{
                    ...
                    <div className="min-w-0 ml-1">
                      <h2 className="font-bold text-[15px] sm:text-lg text-white truncate leading-tight">
                        {activeRoom.name}
                      </h2>
                  ...
                  <AnimatePresence>
                    {replyingTo && (
                      <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: 5 }}
                        className="flex items-center justify-between bg-[#1E293B] border border-slate-700 rounded-lg p-2 px-3"
                      >
                        <div className="min-w-0 pr-4 border-l-2 border-[#009c3b] pl-2 opacity-80">
                          <span className="text-xs font-bold text-[#009c3b] block truncate flex gap-1 items-center">
                            <Reply className="w-3 h-3" /> Repondendo a{" "}
                            {replyingTo.name}
                          </span>
      ...
      {!activeRoomId && (
        <div className="hidden md:flex flex-1 items-center justify-center bg-[#050B14]">
          <div className="flex flex-col items-center text-slate-500 max-w-sm text-center">
            ...
            <p className="text-sm">
              Selecione uma sala à esquerda para interagir com ginastas,
              árbitros e membros da equipe técnica.
            </p>
          </div>
        </div>
      )}
          ...
          <AnimatePresence>
            {roomToDelete && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="fixed inset-0 z-[99999] flex items-center justify-center p-4 sm:p-6 bg-black/90 backdrop-blur-sm"
              >
                <motion.div
                  initial={{ scale: 0.95 }}
                  animate={{ scale: 1 }}
                ...
                >
                  ...
                  <p className="text-slate-400 text-sm mb-8 text-center break-words">
                    ...
                    <b>{roomToDelete.name}</b>?
                    <br className="hidden sm:block" /> Todas as mensagens serão
                    perdidas permanentemente.
                  </p>
                  <div className="flex flex-col sm:flex-row gap-3 w-full mt-auto">
                    <button
                      onClick={() => setRoomToDelete(null)}
                      className="w-full px-5 py-3 rounded-xl font-bold text-slate-300 hover:bg-white/5 transition-colors border border-transparent hover:border-slate-700"
                    >
                      Cancelar
                    </button>
                    <button
                      onClick={handleDeleteRoom}
                      className="w-full px-5 py-3 rounded-xl font-bold bg-red-500 text-white hover:bg-red-600 transition-colors shadow-[0_0_15px_rgba(239,68,68,0.3)]"
                    >
                      Excluir
                    </button>
                  </div>
                ...
                >
                  <div className="p-5 border-b border-slate-700 font-bold text-lg flex justify-between items-center text-white shrink-0">
                    ...
                    <button
                      ...
                      className="text-slate-400 hover:text-white p-1.5 rounded-full hover:bg-white/10 transition-colors focus:outline-none mb-0"
                    >
                  ...
                  <form
                    ...
                    className="p-4 space-y-4 max-h-[85vh] overflow-y-auto w-full"
                  >
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-[#1E293B] border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#009c3b]"
                        placeholder="Ex: Geral, Dúvidas..."
                      />
                    </div>
                      ...
                      <div className="relative z-[65]">
                        ...
                        >
                          <span className="truncate">
                            {liveCompetitions.find(
                              (c) => c.name === roomForm.name,
                            )?.name || "Selecione (Opcional)"}
                          </span>
                          <ChevronDown
                            className={`w-4 h-4 text-slate-400 transition-transform ${isLiveEventDropdownOpen ? "rotate-180" : ""}`}
                          />
                        </button>
                        <AnimatePresence>
                          {isLiveEventDropdownOpen && (
                            <motion.div
                              ...
                              animate={{ opacity: 1, y: 0 }}
                              exit={{ opacity: 0, y: -5 }}
                              className="absolute z-[75] w-full mt-1 bg-[#1E293B] border border-slate-600 rounded-xl shadow-xl overflow-hidden py-1 max-h-48 overflow-y-auto w-[100%] max-w-full"
                            >
                              <div
                                onClick={() => {
                                  setIsLiveEventDropdownOpen(false);
                                }}
                                className={`px-4 py-2.5 flex items-center justify-between cursor-pointer hover:bg-white/5 transition-colors text-slate-400`}
                              >
                                Nenhum
                              </div>
                      ...
                      <AnimatePresence>
                        {isCategoryDropdownOpen && (
                          <motion.div
                            ...
                            animate={{ opacity: 1, y: 0 }}
                            exit={{ opacity: 0, y: -5 }}
                            className="absolute z-[70] w-full mt-1 bg-[#1E293B] border border-slate-600 rounded-xl shadow-xl overflow-hidden py-1"
                          >
                            {CATEGORIES.map((c) => (
                              <div
                                key={c}
                                onClick={() => {
                                  setRoomForm({ ...roomForm, category: c });
                                  setIsCategoryDropdownOpen(false);
                                }}
                                className={`px-4 py-2.5 flex items-center justify-between cursor-pointer hover:bg-white/5 transition-colors ${roomForm.category === c ? "text-[#009c3b] font-bold bg-[#009c3b]/10" : "text-slate-200"}`}
                              >
                                {c}
                                {roomForm.category === c && (
                                  <Check className="w-4 h-4" />
                      ...
                      >
                        <span className="flex items-center gap-2">
                          <span
                            ...
                            style={{ backgroundColor: roomForm.color }}
                          ></span>
                          {
                            PRESET_COLORS.find(
                              (c) => c.class === roomForm.color,
                            )?.name
                          }
                        </span>
                        <ChevronDown
                          className={`w-4 h-4 text-slate-400 transition-transform ${isColorDropdownOpen ? "rotate-180" : ""}`}
                        />
                      </button>
                      <AnimatePresence>
                        {isColorDropdownOpen && (
                          <motion.div
                            ...
                            animate={{ opacity: 1, y: 0 }}
                            exit={{ opacity: 0, y: -5 }}
                            className="absolute z-[70] w-full mt-1 bg-[#1E293B] border border-slate-600 rounded-xl shadow-xl overflow-hidden py-1 max-h-48 overflow-y-auto w-[100%] max-w-full"
                          >
                            {PRESET_COLORS.map((c) => (
                              <div
                                key={c.class}
                                onClick={() => {
                                  setRoomForm({ ...roomForm, color: c.class });
                                  setIsColorDropdownOpen(false);
                                }}
                                className={`px-4 py-2.5 flex items-center justify-between cursor-pointer hover:bg-white/5 transition-colors ${roomForm.color === c.class ? "bg-white/5 font-bold" : "text-slate-200"}`}
                              >
                                <span className="flex items-center gap-2">
                                  <span
                                    className="w-3.5 h-3.5 rounded-full"
                    ...
                    <div className="pt-2 pb-1 relative z-10 w-full overflow-hidden">
                      <label className="flex items-center gap-2 text-sm text-slate-300 cursor-pointer w-fit max-w-[100%]">
                        <input
                          ...
                          className="rounded bg-[#1E293B] border-slate-700 text-[#009c3b] focus:ring-[#009c3b] w-4 h-4 shrink-0"
                        />
                        <span className="truncate">
                          Marcar como evento ao vivo
                        </span>
                      </label>
                    </div>
                    <div className="pt-4 flex justify-end gap-3 border-t border-slate-700/50 mt-4 relative z-10">
                      <button
                        type="button"
                        onClick={() => setIsNewRoomModalOpen(false)}
                        className="px-5 py-2.5 rounded-xl font-bold text-slate-300 hover:bg-white/5 transition-colors w-full sm:w-auto"
                      >
                        Cancelar
                      </button>
                      <button
                        type="submit"
                        className="px-5 py-2.5 rounded-xl font-bold bg-[#009c3b] text-white hover:bg-green-600 transition-colors shadow-[0_0_15px_rgba(0,156,59,0.3)] w-full sm:w-auto"
                      >
                        Criar Sala
                      </button>
                    </div>
