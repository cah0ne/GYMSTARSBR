...
import { Shapes, Plus, Search, ChevronRight, Pencil, Trash2, LayoutGrid, X, ChevronDown } from "lucide-react";
...
function CustomSelect({
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      {required && (
         <input type="text" readOnly className="sr-only" required value={value} />
      )}
      <button
        type="button"
      ...
      >
        ...
        <div className="flex items-center shrink-0 transition-opacity">
           {iconComponent || <ChevronDown className={`w-4 h-4 text-slate-500 transition-transform ${isOpen ? 'rotate-180' : ''}`} />}
        </div>
      </button>
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-20">
      ...
      <div className="flex justify-between items-center bg-slate-900 border border-slate-800 p-6 rounded-3xl shadow-xl shadow-black/20">
        <h1 className="text-2xl sm:text-3xl font-black text-white italic uppercase tracking-tighter">Elementos</h1>
        {isAdmin && (
          <button
            onClick={() => {
              resetForm();
             ...
             {app.icon ? (
               <img 
                 ...
                 style={{ 
                   width: `${iconSize}px`, 
                   height: `${iconSize}px`,
                   filter: activeApparatus === app.id ? "brightness(1.1) contrast(1)" : "brightness(0.65)"
                 }}
               />
             ) : (
               <div 
               ...
               >
                 {app.id === "solo" ? "SL" : app.id === "salto" ? "ST" : app.id === "paralelas" ? "PR" : "TR"}
               </div>
             )}
         ...
         <div className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="relative w-full sm:w-48 z-40">
               <CustomSelect
                  ...
                  options={(GROUPS_BY_APPARATUS[activeApparatus] || []).map(g => ({ value: g, label: g }))}
                  placeholder="GRUPO"
                  iconComponent={<></>}
               />
            </div>
          ...
          {filteredContents.map((item, i) => (
             ...
             >
                ...
                <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-xl overflow-hidden bg-black shrink-0 border border-slate-800 flex items-center justify-center">
                   {item.imageUrl ? (
                      <img src={item.imageUrl} alt="" className="w-full h-full object-cover select-none pointer-events-none" draggable="false" onContextMenu={(e) => e.preventDefault()} />
                   ) : item.videoUrl && getYoutubeVideoId(item.videoUrl) ? (
                      <img src={`https://img.youtube.com/vi/${getYoutubeVideoId(item.videoUrl)}/mqdefault.jpg`} alt="" className="w-full h-full object-cover select-none pointer-events-none" draggable="false" onContextMenu={(e) => e.preventDefault()} />
                   ) : (
                     ...
                     <div className="pr-4">
                        <div className="flex items-center gap-2 mb-2">
                           <span className="text-xs font-black uppercase tracking-widest text-[#00a84c] flex items-center gap-1.5 font-bold">
                             {(() => {
                               const foundOpt = apparatusOptions.find(a => a.id === selectedElement.apparatus);
                               return (
                                 <>
                                   {foundOpt?.icon && (
                                     <img src={foundOpt.icon} alt="" className="w-4 h-4 object-contain inline select-none pointer-events-none" />
                                   )}
                  ...
                  <div className="p-6 sm:p-8 overflow-y-auto space-y-8 bg-slate-900 custom-scrollbar">
                     <div className="flex gap-3">
                        {selectedElement.difficulty && (
                           <div className="bg-emerald-500/10 border border-emerald-500/20 px-4 py-2 rounded-xl flex items-center gap-2">
                              ...
                              <span className="text-sm font-bold text-emerald-400">Dificuldade [{selectedElement.difficulty}]</span>
                           </div>
                        )}
                        {selectedElement.group && (
                           <div className="bg-purple-500/10 border border-purple-500/20 px-4 py-2 rounded-xl">
                              <span className="text-sm font-bold text-purple-400">{selectedElement.group}</span>
                           </div>
                        )}
                     </div>
                     ...
                     {selectedElement.text && (
                        <div className="prose prose-invert prose-p:leading-relaxed prose-p:text-slate-300 max-w-none text-sm sm:text-base">
                           <p className="whitespace-pre-wrap">{selectedElement.text}</p>
                        </div>
                     )}

                     {selectedElement.videoUrl && getYoutubeVideoId(selectedElement.videoUrl) && (
                        <div className="space-y-3">
                           <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest">Vídeo Demonstrativo</h3>
                     ...
                     {selectedElement.imageUrl && (
                        <div className="space-y-3">
                           <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest">Esboço / Foto</h3>
                           <div className="rounded-2xl overflow-hidden border border-slate-700 bg-black">
                              <img src={selectedElement.imageUrl} alt="" className="w-full h-auto max-h-[500px] object-contain select-none pointer-events-none" draggable="false" onContextMenu={(e) => e.preventDefault()} />
                           </div>
                        </div>
                     )}
                  </div>
                  ...
                  <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-6 space-y-5 custom-scrollbar">
                     <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="z-[60] relative">
                           ...
                           <CustomSelect
                              ...
                              options={apparatusOptions.map(a => ({ value: a.id, label: a.name }))}
                        ...
                        <div className="z-[50] relative">
                           ...
                           <CustomSelect
                              ...
                              onChange={(val) => setNewContent({ ...newContent, difficulty: val })}
                              options={DIFFICULTY_OPTIONS}
                     ...
                     <div className="z-[40] relative">
                        ...
                        <CustomSelect
                           ...
                           onChange={(val) => setNewContent({ ...newContent, group: val })}
                           options={(GROUPS_BY_APPARATUS[newContent.apparatus] || []).map(g => ({ value: g, label: g }))}
                     ...
                     <div className="space-y-4 pt-2">
                        <h3 className="text-sm font-bold text-white border-b border-slate-800 pb-2">Mídia</h3>
                        <div>
                           <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">
