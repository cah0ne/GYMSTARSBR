    ...
    <div className="flex flex-col md:flex-row h-screen w-full bg-[#050B14] text-slate-100 font-sans selection:bg-[#009c3b] selection:text-white overflow-hidden">
      ...
      <nav className="hidden md:flex flex-col w-24 lg:w-64 border-r border-slate-800 bg-[#070F1C] overflow-y-auto">
        ...
        <div className="flex-1 px-4 py-2 space-y-6 overflow-y-auto hidden-scrollbar">
          ...
          <div className="space-y-1">
            <p className="px-3 text-[10px] font-black uppercase tracking-widest text-[#009c3b]/70 mb-3 hidden lg:block">
              Principal
            </p>
              ...
              >
                {isActive(item.path) && (
                  <motion.div
                    ...
                    className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-[#009c3b] rounded-r-full"
                  />
                )}
                <div className="relative z-10 flex items-center space-x-3 transition-transform duration-300 group-hover:translate-x-1">
                  {branding?.menuIcons?.[item.id] ? (
                    <img
                      src={branding.menuIcons[item.id] || undefined}
                      alt=""
                      className="object-contain -ml-0.5"
                      style={{
                        width: branding.menuIconsSize || 24,
                        height: branding.menuIconsSize || 24,
                      }}
                    />
                  ) : (
          ...
          {(showChat ||
            ...
            userData?.tag === "Admin") && (
            <div className="space-y-1">
              <p className="px-3 text-[10px] font-black uppercase tracking-widest text-white/30 mb-3 hidden lg:block">
                Sistema
              </p>
              {showChat && (
                <Link
                ...
                >
                  ...
                  <div className="relative z-10 flex items-center space-x-3 transition-transform duration-300 group-hover:translate-x-1">
                    {branding?.menuIcons?.["batePapo"] ? (
                      <img
                        src={branding.menuIcons["batePapo"]}
                        alt=""
                        className="object-contain -ml-0.5"
                        style={{
                          width: branding.menuIconsSize || 24,
                    ...
                    <span className="font-medium hidden lg:inline flex-1 text-sm">
                      Bate-papo
                    </span>
                  </div>
                  {unreadCount > 0 && (
              ...
              {(userData?.tag === "Árbitro" || userData?.tag === "Admin") && (
                ...
                >
                  {isActive("/referee") && (
                    <motion.div
                      layoutId="desktop-active"
                      className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-yellow-500 rounded-r-full"
                    />
                  )}
                  <div className="relative z-10 flex items-center space-x-3 transition-transform duration-300 group-hover:translate-x-1">
                    {branding?.menuIcons?.["arbitragem"] ? (
                      <img
                        src={branding.menuIcons["arbitragem"]}
                        alt=""
                        className="object-contain -ml-0.5"
                        style={{
                          width: branding.menuIconsSize || 24,
        ...
        <div className="p-4 border-t border-slate-800">
          <div className="lg:bg-slate-800/50 lg:p-3 rounded-2xl flex flex-col lg:flex-row items-center space-y-2 lg:space-y-0 lg:space-x-3">
            <Link
              to="/profile"
              className="w-10 h-10 rounded-full bg-gradient-to-br from-yellow-400 to-[#009c3b] flex-shrink-0 border-2 border-white/20 flex items-center justify-center overflow-hidden"
            >
              {userData?.photoURL ? (
                <img
            ...
            <div className="overflow-hidden hidden lg:block">
              ...
              <span className="text-[10px] uppercase tracking-widest text-[#009c3b] font-black">
                {userData?.tag}
              </span>
            </div>
          </div>
      ...
      <main className="flex-1 flex flex-col min-h-0 overflow-hidden relative">
        ...
        <header className="md:hidden h-14 border-b border-white/5 flex items-center justify-between px-4 bg-[#050B14] shrink-0 z-50">
          <Link
            to="/profile"
            className={`w-8 h-8 rounded-full overflow-hidden border border-white/10 flex items-center justify-center shrink-0 ${isActive("/profile") ? "ring-2 ring-[#009c3b] ring-offset-2 ring-offset-[#050B14]" : ""}`}
          >
            {userData?.photoURL ? (
              <img
                src={userData.photoURL || undefined}
              ...
              <UserCircle
                className={`w-5 h-5 ${isActive("/profile") ? "text-[#009c3b]" : "text-slate-400"}`}
              />
            )}
          </Link>
          ...
          {mobileMenuOpen && (
            ...
            >
              <div className="px-4 py-4 flex flex-col space-y-6 max-h-[80vh] overflow-y-auto hidden-scrollbar">
                {/* Main / Core */}
                <div className="space-y-1">
                  <p className="px-3 text-[10px] font-black uppercase tracking-widest text-[#009c3b]/70 mb-2">
                    Principal
                  </p>
                  {mainNavItems.map((item) => (
                    <Link
                    ...
                    >
                      ...
                      {branding?.menuIcons?.[item.id] ? (
                        <img
                          src={branding.menuIcons[item.id] || undefined}
                          alt=""
                          className="object-contain -ml-0.5 relative z-10"
                          style={{
                            width: branding.menuIconsSize || 24,
                            height: branding.menuIconsSize || 24,
                          }}
                ...
                {(showChat ||
                  ...
                  <div className="space-y-1 pb-4">
                    <p className="px-3 text-[10px] font-black uppercase tracking-widest text-white/30 mb-2">
                      Sistema
                    </p>
                    {showChat && (
                      <Link
...
export default function App() {
  ...
  if (loading) {
    ...
    return (
      <div
        className={`min-h-screen flex items-center justify-center ${isOverlay ? "bg-transparent" : "bg-[#050B14] text-white"}`}
      >
