...
export default function GymStarsLogo({
  ...
  const [customLogo, setCustomLogo] = useState<{
    ...
    try {
      const b64 = localStorage.getItem(
        `gymstars_custom_logo_base64_${section}`,
      );
  ...
  const [brandingConfig, setBrandingConfig] =
    useState<any>(brandingConfigCache);
  ...
  useEffect(() => {
    ...
    if (!base64 && !url) {
      if (section !== "home" && fallbackData) {
        ...
        base64 =
          fallbackData.authLogoBase64 || fallbackData.sidebarLogoBase64 || "";
  ...
  if (variant === "vertical") {
    return (
      <div
        ...
        id="gymstars-logo-vertical"
      >
        <GymnastIcon />
        <div className="mt-2 select-none">
          <div
            className={`${selectedSize.text} font-black tracking-widest leading-none text-white font-sans`}
          >
            GYMSTARS
          </div>
          <div
            className={`${selectedSize.subtitle} font-extrabold tracking-[0.3em] font-sans text-[#009c3b] uppercase leading-none mt-1`}
          >
            BRASIL
          </div>
        </div>
  ...
  return (
    <div
      ...
      id="gymstars-logo-horizontal"
    >
      <GymnastIcon />
      <div className="select-none leading-none">
        <div
          className={`${selectedSize.text} font-black tracking-wider text-white font-sans`}
        >
          GYMSTARS
        </div>
        <div
          className={`${selectedSize.subtitle} font-bold tracking-[0.25em] text-[#009c3b] uppercase mt-0.5`}
        >
          BRASIL
        </div>
      </div>
